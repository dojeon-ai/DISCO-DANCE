...
import envs.dmc_suite.custom_dmc_tasks as cdmc
...
def _make_dmc(obs_type, domain, task, frame_stack, action_repeat, seed, maximum_timestep):
    visualize_reward = False
    if (domain, task) in suite.ALL_TASKS:
        env = suite.load(domain,
                         task,
                         task_kwargs=dict(random=seed, time_limit=maximum_timestep),
...
def make(name, obs_type, frame_stack, action_repeat, seed, guide_env=False):
    ...
    if guide_env:
        maximum_timestep = float("inf")
    else:
        maximum_timestep = 1000

    env = make_fn(obs_type, domain, task, frame_stack, action_repeat, seed, maximum_timestep)
