...
class VideoRecorder:
    ...
    def init(self, env, enabled=True):
        self.frames = []
        self.enabled = self.save_dir is not None and enabled and (not self.maze_parent=='2dmaze')
        self.record(env)


    def record(self, env):
        if self.enabled:
            frame = env._env.physics.render(height=self.render_size,
